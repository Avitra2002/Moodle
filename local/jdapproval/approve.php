...
require_capability('local/jdapproval:reviewjd', $systemctx);
global $DB, $OUTPUT, $PAGE, $USER;
$PAGE->set_context($systemctx);
$PAGE->set_url(new moodle_url('/local/jdapproval/approve.php'));
$PAGE->set_title('JD Approval Panel');
$PAGE->set_heading('JD Approval Panel');
// Function to send bell notification and message to user on JD status change
function notify_user_jd_status_change(int $userid, string $status, stdClass $adminuser) {
    global $DB;
    $user = $DB->get_record('user', ['id' => $userid], '*', MUST_EXIST);
    $subject = 'JD ' . ucfirst($status);
    $fullmessage = "Hello " . fullname($user) . ",\n\nYour JD has been {$status} by the administrator.\n\nRegards,\n" . fullname($adminuser);
    $from = core_user::get_support_user();
    $message = new \core\message\message();
    $message->component         = 'local_jdapproval';      // Your plugin/component name
    $message->name              = 'jd_status_change';    // Arbitrary internal message name
...
// Get field IDs
$cvfieldid = $DB->get_field('user_info_field', 'id', ['shortname' => 'JD_Upload']);
$statusfieldid = $DB->get_field('user_info_field', 'id', ['shortname' => 'jd_status']);
...
if ($userid && in_array($action, ['approve', 'reject'])) {
    ...
    notify_user_jd_status_change($userid, $status, $USER);
    redirect(new moodle_url('/local/jdapproval/approve.php'), "User status updated and notified.");
}
/**
...
$table->head = ['Name', 'Email', 'JD File', 'Status', 'Action'];
   ...
   $filearea = 'files_' . $cvfieldid;
    ...
    foreach ($files as $file) {
        if (!$file->is_directory()) {
            ...
            $cvlink = html_writer::link($cvurl, '📎 View JD');
    ...
    if ($status === 'approved') {
        ...
        $approveurl = new moodle_url('/local/jdapproval/approve.php', ['userid' => $user->id, 'action' => 'approve']);
        $rejecturl = new moodle_url('/local/jdapproval/approve.php', ['userid' => $user->id, 'action' => 'reject']);
        $actions = html_writer::link(
            $approveurl,
            '✅ Approve',
            ['onclick' => "return confirm('Are you sure you want to approve this JD?');"]
        ) . ' | ' .
        html_writer::link(
            $rejecturl,
            '❌ Reject',
            ['onclick' => "return confirm('Are you sure you want to reject this JD?');"]
        );
    }
    $table->data[] = [
        fullname($user),
