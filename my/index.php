...
if (is_siteadmin($USER)) {
    redirect(new moodle_url('/local/customdashboard/index.php'));
    exit;
}
...
// Loop through courses and check roles
foreach ($courses as $course) {
    $coursecontext = context_course::instance($course->id);
    if (user_has_role_assignment($userid, 4, $coursecontext->id) || user_has_role_assignment($userid, 1, $coursecontext->id)) { // 3 = editingteacher
        $is_teacher = true;
        break;
    }
    if (user_has_role_assignment($userid, 5 , $coursecontext->id)) { // 5 = student
        $is_student = true;
        // Keep checking in case user is teacher too
    }
}

// Redirect accordingly
if ($is_teacher) {
    redirect(new moodle_url('/local/customdashboard/trainer_dashbaord1.php'));
    exit;
} elseif ($is_student) {
    redirect(new moodle_url('/local/customdashboard/studentdashbaord.php'));
    exit;
}
