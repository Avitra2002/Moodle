...
class profile_field_file extends profile_field_base {
    ...
    if (!isset($usernew->{$this->inputname})) {
        // Field not present — probably from bulk CSV. Skip to preserve file.
        return;
    }

    $draftitemid = $usernew->{$this->inputname};
    $usercontext = context_user::instance($this->userid, MUST_EXIST);
    $component = 'profilefield_file';
    $filearea = 'files_' . $this->fieldid;

    $fs = get_file_storage();

    // Draft files submitted
    ...
    // 🧠 Key condition:
    // Only skip saving if:
    // - No draft file uploaded
    // - Existing file exists
    // - AND this is a backend/CSV context
    if (!$hasdraftfile && $hasexistingfile && !defined('USER_EDITFILE_ALLOWED')) {
        error_log("🛡️ Preserving existing file — no new file submitted and no delete intent.");
        return;
    }

    // ✅ Save new files or delete if empty draft submitted
