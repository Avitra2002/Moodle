...
function theme_almondb_extend_primary_navigation(\navigation_primary $navigation) {
    global $USER;
    if (isloggedin() && !isguestuser()) {
        $url = new moodle_url('/local/myreport/index.php');
        $node = $navigation->add(
            get_string('myreport', 'local_myreport'),
            $url,
